...
def process_normal_data(data, session):
    ...
    if data.HasField("observation"):
        ...
        session._servicer.rewards_counter.labels(session.name, session.impl_name).inc()
        if value < 0.0:
            session._servicer.rewards_received.labels(session.name, session.impl_name).dec(abs(value))
        else:
            session._servicer.rewards_received.labels(session.name, session.impl_name).inc(value)

    elif data.HasField("message"):
        recv_event.messages = [RecvMessage(data.message)]
        session._new_event(recv_event)

        session._servicer.messages_received.labels(session.name, session.impl_name).inc()
...
async def process_incoming(context, session):
    try:
        session._servicer.actors_started.labels(session.impl_name).inc()
        ...
        while True:
            ...
            if data.state == common_api.CommunicationState.NORMAL:
                ...
                session._servicer.actors_ended.labels(session.impl_name).inc()
                break
...
async def send_data(context, session):
    try:
        while True:
...
class AgentServicer(grpc_api.ServiceActorSPServicer):
    ...
    def _start_session(self, trial_id, init_input):
        ...
        new_session = _ServedActorSession(impl.impl, actor_class, trial, actor_name, impl_name, config)
        new_session._servicer = self
    ...
    async def _run_session(self, context, session):
        ...
        try:
            with self.decide_request_time.labels(session.name, session.impl_name).time():
                send_task = asyncio.create_task(send_data(context, session))
                process_task = asyncio.create_task(process_incoming(context, session))
